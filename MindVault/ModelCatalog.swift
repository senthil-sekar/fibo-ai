...
///
/// IMPORTANT: EmbeddingGemma's encoder is not yet implemented in MLXEmbedders
/// (only BERT / Nomic BERT / Qwen3 architectures are supported as of this build),
/// so we default to Nomic Embed Text v1.5 — a retrieval-tuned 768-dim model that
/// IS supported. When the Gemma encoder lands in MLXEmbedders, add a
/// `.embeddingGemma` case here and point it at `mlx-community/embeddinggemma-300m-*`.
///
...
enum EmbeddingModelOption: String, CaseIterable, Identifiable, Codable {
    ...
    /// Some embedding models are trained with task-specific instruction
    /// prefixes. Applying the right prefix to queries vs. stored documents
